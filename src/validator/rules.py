...
class PriceRangeRule(ValidationRule):
    ...
    def __init__(self, field: str, value: float, min_price: float, max_price: float, **kwargs):
        ...
        message = (
            f"Field '{field}' must be between {min_price} and {max_price}." if not is_valid else ""
        )
        super().__init__(field=field, message=message, is_valid=is_valid, **kwargs)
...
class StockQuantityRule(ValidationRule):
    ...
    def __init__(self, field: str, value: int | None, **kwargs):
...
class ImageValidationRule(ValidationRule):
    ...
    def __init__(self, field: str, value: str | None, **kwargs):
        ...
        message = (
            f"Field '{field}' must be a valid image URL with dynamic path (e.g., /uploads/2026/07/...)."
            if not is_valid
            else ""
        )
        super().__init__(field=field, message=message, is_valid=is_valid, **kwargs)
