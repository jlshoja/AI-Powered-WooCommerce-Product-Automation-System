...
def test_ai_client():
    ...
    with patch.object(ai_client.client.chat.completions, "create") as mock_create:
        ...
        assert seo_description == "توضیحات سئو بهینه", (
            f"Expected 'توضیحات سئو بهینه', got {seo_description}"
        )
...
def test_ai_manager():
    ...
    with (
        ...
        assert processed_product.seo_title == "عنوان سئو بهینه", (
            f"Expected 'عنوان سئو بهینه', got {processed_product.seo_title}"
        )
        assert processed_product.seo_description == "توضیحات سئو بهینه", (
            f"Expected 'توضیحات سئو بهینه', got {processed_product.seo_description}"
        )
        assert processed_product.description == "توضیحات کامل محصول", (
            f"Expected 'توضیحات کامل محصول', got {processed_product.description}"
        )
        assert processed_product.tags == ["چرم", "کیف مردانه"], (
            f"Expected ['چرم', 'کیف مردانه'], got {processed_product.tags}"
        )
        assert processed_product.categories == ["کیف مردانه", "اکسسوری"], (
            f"Expected ['کیف مردانه', 'اکسسوری'], got {processed_product.categories}"
        )
...
def test_batch_importer():
    ...
    assert len(batch_importer.tracker.imported_products) == 1, (
        f"Expected 1 imported product, got {len(batch_importer.tracker.imported_products)}"
    )
    assert batch_importer.tracker.imported_products[0]["sku"] == "TEST001", (
        f"Expected 'TEST001', got {batch_importer.tracker.imported_products[0]['sku']}"
    )
...
def test_progress_tracker():
    ...
    assert len(tracker.imported_products) == 1, (
        f"Expected 1 imported product, got {len(tracker.imported_products)}"
    )
    assert len(tracker.failed_products) == 1, (
        f"Expected 1 failed product, got {len(tracker.failed_products)}"
    )
    ...
    with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp_file:
        ...
        assert Path(tmp_file.name).exists(), (
            f"Expected report at {tmp_file.name}, but file does not exist"
        )
